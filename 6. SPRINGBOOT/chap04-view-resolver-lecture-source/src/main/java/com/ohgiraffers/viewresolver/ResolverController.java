...
public class ResolverController {
    ...
    /* 설명. 스프링의 RedirectAttributes라는 객체에 attr을 담으면 리다이렉트 시에도 값(전달할 상태)이 유지 된다. */
    ...
    /* 설명. 단순 forward 시에는 String 반환과 Model을 활용하는 코드와 ModelAndView를 활욯나 코드가 차이가 없다. */
    @GetMapping("modelandview")
    public ModelAndView modelAndViewTest(ModelAndView mv) {
        mv.addObject("message2", "ModelAndView를 이용한 forwaard");
