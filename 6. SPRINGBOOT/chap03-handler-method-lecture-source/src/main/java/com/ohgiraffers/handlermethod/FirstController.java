...
public class FirstController {
    ...
    @GetMapping("regist")   // '/regist' 또는 'regist' 둘 다 가능
...
//    public String registMenu(HttpServletRequest request) {
     ...
     *  @RequestParam 어노테이션을 생략도 가능하다.
     *  (스프링부터는 컨트롤러의 파싱(Parsing)작업이 간소화 된다는 것을 알 수 있다.)
    * */

    /* 설명.
     *  @RequestParam의 속성들
     *  1. defaultValue: 사용자의 입력 값이 없거나("") 아니면 request의 parameter 키 값과 일치하지 않는 매개변수 명 사용시 매개변수가 기질 기본 default 값을 작성한다.
    ...
    * */
    ...
    /* 설명. @ModelAttribute 어노테이션을 활용하면 커맨드 객체를 모델에도 담아주며 어트리뷰트의 키값을 지정할 수 있다.(키 값이 없으면 타입의 낙타봉 표기법이 키 값이다.) */
    @PostMapping("search")
    public String seachMenu(@ModelAttribute("menu") MenuDTO menu) {
